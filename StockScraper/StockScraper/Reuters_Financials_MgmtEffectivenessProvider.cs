...
{
    public class reuters_Financials_MgmtEffectivenessProvider
    {
        public static List<reuters_Financials_MgmtEffectiveness> GetData(HtmlDocument doc, int job_id,int scheduler_id, int stock_id,string URL)
        {
            List<reuters_Financials_MgmtEffectiveness> rType = new List<reuters_Financials_MgmtEffectiveness>();
            ...
            if (rowcollection != null)
            {
                if (rowcollection.Count > 0)
                {
                    ...
                    var tblrows = rowcollection[8].SelectNodes("div[@class='moduleBody']//table[@class='dataTable']//tr");
                    if (tblrows != null)
                    {
                        for (int k = 1; k < tblrows.Count; k++)
                        {
                            HtmlNode tr = tblrows[k];
                            reuters_Financials_MgmtEffectiveness temp = new reuters_Financials_MgmtEffectiveness();
            ...
            {
                string warningmsg = Helper.GetWarningMSG(stock_id, "reuters_Financials_MgmtEffectiveness", URL);
